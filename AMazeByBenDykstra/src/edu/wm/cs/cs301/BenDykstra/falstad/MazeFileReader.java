...
public class MazeFileReader extends AsyncTask<Void, Void, Integer>{
	 ...
	 *  
	 * @param fs with data of a Maze object
	 */
	public MazeFileReader(FileInputStream fs, Maze m) {
		//load(fs) ;
		maze = m;
		file = fs;
		execute();
	...
	protected Integer doInBackground(Void... params) {
		Log.v("!!!!!!", "Loading a maze from file");
		load(file);
		return 0;
	}
	
	/**
	 * sets the maze height, width, distances, and initializes it. sets the progress bar to 100 so that they play button shows
	 */
	protected void onPostExecute(Integer returnVal){
		//System.out.println("Called onPostExecute");
		Log.v("!!!!!!", "in onPostExecute");
		done = true;
		maze.getGenAct().finishLoad();;
		maze.getGenAct().getProgressBar().setProgress(100);
		maze.getGenAct().setText(100);
		maze.getGenAct().playButton.setVisibility(View.VISIBLE);
		//maze.getGenAct().startPlay();
	}
	/**
	 * Method provides main functionality to read all attributes of maze object from the given file
	 * @param fs gives the input file
	 */
	private void load(FileInputStream fs)
	{
		Log.v("!!!!!!" , "in load, called from doInBackGround");
		try{

			//File fXmlFile = new File(fs);
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		//GraphicsWrapper.getColorValues(result.col);
		//result.col = GraphicsWrapper.getColorValuesWithInt(col);
