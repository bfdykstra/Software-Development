...
import edu.wm.cs.cs301.BenDykstra.falstad.GraphicsWrapper.gwColor;
...
public class FirstPersonDrawer extends DefaultViewer {
	...
	public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells mazecells, Cells seencells, int map_scale, int[][] mazedists, int mazew, int mazeh, BSPNode bsp_root, Maze maze){
		...
		gw = edu.wm.cs.cs301.BenDykstra.ui.PlayActivity.gw; 
	}
	 ...
	 */
	@Override
	public void redraw(GraphicsWrapper gc, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang, boolean stopped) {
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
		//Color wierdCol = seg.col;
		
		gw.setColorWithArray(seg.col);
		//gw.setColor(gwColor.blue);
		boolean drawn = false;
