...
public class Cells {
	...
	private boolean isFirstVisit(int x, int y) {
		boolean t = hasMaskedBitsTrue(x, y, Constants.CW_VISITED);
	...
	public boolean hasMaskedBitsTrue(int x, int y, int bitmask) {
		boolean t = (cells[x][y] & bitmask) != 0;
		//return (cells[x][y] & bitmask) != 0;
		return t;
	}
	public boolean hasMaskedBitsFalse(int x, int y, int bitmask) {
		try{ return (cells[x][y] & bitmask) == 0;
		
		} catch(ArrayIndexOutOfBoundsException e){
			Log.v("Array out bounds exception!!!!", "x coordinate: " + x + "y coordinate: " + y);
			return false;
		}
