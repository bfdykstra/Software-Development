...
public class MazeBuilder extends AsyncTask<Void, Void, Integer> {
	...
	@Override
	/**
	 * basically run method of old mazebuilder, now refactored for async task. Called when execute is called in build().
	 */
	protected Integer doInBackground(Void... params) {
		...
		try {
			...
			//maze.newMaze(root, cells, dists, 0, 0);
	...
	protected void onProgressUpdate(Void... values) {
		...
		/*maze.getGenAct().getProgressBar().setProgress(maze.getPercentDone());
		maze.getGenAct().setText(maze.getPercentDone());*/
	}
	
	/**
	 * get progress bar to finally display 100 for christ sake
	 */
