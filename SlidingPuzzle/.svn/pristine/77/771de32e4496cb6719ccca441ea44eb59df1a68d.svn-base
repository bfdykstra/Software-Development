...
public class SimplePuzzleState implements PuzzleState {
	...
	public boolean equals(Object other){
		PuzzleState otherPuz = (PuzzleState) other;
		...
		// If lengths are not equal, the two grids cannot be the same
		if (this.getState().length != otherPuz.getState().length){
			return false;
		}
		
		if (Arrays.equals(this.getState(), otherPuz.getState())){
			return true;
		}
	...
	public int hashCode(){
		...
		for(i = 0; i < puzzle.length; i++){
			weightedSum = weightedSum + puzzle[i]*(1 + i); 
		}
		return weightedSum;
